...
function changeSelect()
{
	...
	else if(selectclass=='景點')
	{
		...
		light.selected = "${(SpotVO.spotsort=='日景')?'true':''}";
		night.selected = "${(SpotVO.spotsort=='夜景')?'true':''}";
	...
	else if(selectclass=='美食')
	{
		...
		snack.text = "小吃";
		all.selected = "${(SpotVO.spotsort=='無')?'true':''}";
		restaurant.selected = "${(SpotVO.spotsort=='餐廳')?'true':''}";
		snack.selected = "${(SpotVO.spotsort=='小吃')?'true':''}";
		selectsort.add(all);
