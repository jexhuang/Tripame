...
<script>
	...
	$(function() {
		...
		viewArray = new Array();
		tabCount=1;
		tabStatusArray = new Array();//用來儲存使用者上一個點選過的tabs
		tabStatusArray[0]=1;
		$("#departureErr").hide();
		$("#fromErr").hide();
		$("#endErr").hide();
		$( document ).tooltip();//小提示
	});
	//**************************縣市鄉鎮**************************
	$(function() {
		$('#TWmap').twzipcode({
	...
	function tabNumber(e){//replace(/tabStatus/g,i),e=i
		tabStatus=e;//記錄使用者點選哪個tabs	
		tabStatusArray[tabCount]=e;
		tabCount+=1;
	}
	function checkIt(){
		var tmpTab = tabStatusArray[tabStatusArray.length-2];//取回使用者上次使用點選的tabs數值
	...
	function clickArea(area){
		...
		document.getElementById("tmpArea").value= area;//儲存隱藏域的使用者選的地名
		  ...
		  //建立好Get連接與送出請求
			var url="<%=request.getContextPath()%>/Trip/tripplan?map=searchArea&searchOption=" + searched+ "&area=" + area;
		xhr.open("Get", url, true);
		xhr.send(null);
		return false;//取消超連結效果
	}
	function btnTrip() {
...
<body onload="twMap();clickArea('桃園')">
	<!--隱藏域_儲存使用者選的地名 -->
	<input id="tmpArea" type="hidden" value="" />
	<div class="container">
					...
					<div class="col-md-7 column ">
						<div class="form-group">
							...
							<div class="controls controls-row col-md-6 column ">
								...
								<div class="col-md-8 column">
									<div class="input-group ">
										<input type="text" id="to" name="dateEnd" class="form-control"
											style="font-size: 8px;" onclick="clearErrToDate()"> <span
											class="input-group-addon" id="basic-addon1"><span
											class="input-group-btn glyphicon glyphicon-calendar"></span></span>
									</div>
								...
								<div class="col-md-4 column">
									...
									<div class="row clearfix">
										<input id="toTime" name="toTime" class="form-control"
											type="text" class="time" value="23:59" />
										<script>
											$(function() {
												$('#toTime').timepicker();
											});
					...
					<div class="col-md-3 column">
						...
						<div class="row clearfix">
							<div id="addtabs"
								...
								<a href="#" id="my"><img id="checkItOut" src="<%=request.getContextPath()%>/Front/forIndex/img/icons/idea13.jpg"/></a>
								<div id="tabs">
									<ul id="addUl">
								...
								<div>
									<input type="button" class="btn btn-primary" value="規劃行程"
